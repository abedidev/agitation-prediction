...
from utils import count_location_per_day, count_location_per_hour, \
    ...
    add_agitation_next_column, vitals_to_half, count_type_per_half, merge_merged_label_half, \
    add_agitation_next_column_half
...
for idx, (id, group) in enumerate(physiology.sort_values('id').groupby('id')):
    ...
    group_count_quarter = vitals_to_half(group)
    # group_count_quarter.to_csv(os.path.join(destination + '-values', str(idx).zfill(2) + '-' + id + '.csv'), index=False)
    group_count_quarter.to_csv(os.path.join(destination + '-values-half', id + '.csv'), index=False)
...
for idx, (id, group) in enumerate(labels.sort_values('id').groupby('id')):
    ...
    group_count_quarter = count_type_per_half(group)


    # group_count_quarter.to_csv(os.path.join(destination + '-count-quarter', str(idx).zfill(2) + '-' + id + '.csv'), index=False)
    group_count_quarter.to_csv(os.path.join(destination + '-count-half', id + '.csv'), index=False)




root_activity = '/home/ali/PycharmProjects/tihm/activity-count-half'
root_physiology = '/home/ali/PycharmProjects/tihm/physiology-values-half'
root_merged = '/home/ali/PycharmProjects/tihm/merged-half'
...
root_merged = '/home/ali/PycharmProjects/tihm/merged-half'
root_label = '/home/ali/PycharmProjects/tihm/label-count-half'
root_altogether = '/home/ali/PycharmProjects/tihm/altogether-half'
...
for idx, file in enumerate(files):
    ...
    try:
        ...
        dataFrame = merge_merged_label_half(
            mer,
            lab)
...
altogether.to_csv('dataset-raw-half.csv', index=False)

next = add_agitation_next_column_half(altogether)
next = next[next['agitation-next'] != -10]
next.to_csv('dataset-raw-next-half.csv', index=False)
...
len(altogether[['day', 'half']].drop_duplicates())
len(altogether[['id', 'day', 'half']].drop_duplicates())