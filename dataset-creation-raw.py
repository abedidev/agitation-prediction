...
altogether.to_csv('dataset-raw.csv', index=False)

next = add_agitation_next_column(altogether)
next = next[next['agitation-next'] != -10]
next.to_csv('dataset-raw-next.csv', index=False)

# altogether[altogether['agitation'] >= 1][['id', 'date', '6h', 'agitation']].to_csv('agitation.csv')
# labels[labels['type'] == 'agitation'].to_csv('agitation-.csv')

len(altogether)
len(altogether['id'].unique())
len(altogether[['id', 'date']].drop_duplicates())
len(altogether[['date', '6h']].drop_duplicates())
len(altogether[['id', 'date', '6h']].drop_duplicates())