...
dataset = pd.read_csv(os.path.join(root, 'dataset-raw-next-half.csv'))
...
columns = ['id', 'day', 'half', 'back-door', 'bathroom', 'bedroom',
...
data.drop(['id', 'day', 'half'], axis=1, inplace=True)
x = np.array(data)
...
for fold, (train_idx, test_idx) in enumerate(cv.split(x), start=1):
    ...
    target_class = 1
    desired_count = int(class_counts[1] * factor)  # oversample class 1 to 2× its size
    strategy = {cls: count for cls, count in class_counts.items()}
    strategy[target_class] = desired_count
